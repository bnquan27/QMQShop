...
const Compare = {
	...
	async _renderProductSelector(searchTerm) {
		...
		try {
			...
			if (products.length === 0) {
				container.innerHTML =
					'<div style="padding:30px;text-align:center;color:var(--text-muted);">Không tìm thấy sản phẩm</div>';
				return;
			}
			...
			container.innerHTML = '<ul class="cms-product-list">' + products
				.map((p) => {
          ...
          <li>
            <div class="cms-product-card ${added ? "added" : ""}">
              ...
              ${added
								...
								: disabled
									? '<span class="cms-pc-limit"><i class="fa-solid fa-ban"></i> Tối đa ' +
									this.MAX_COMPARE +
									"</span>"
